...
function read(filename) {
  ...
  if (wrap.dependencies) {
    return wrap.dependencies;
  }
  // legacy - convert from long format
  return wrap.components.reduce(function(deps, c) {
    deps[c.name] = c.version;
    return deps;
  }, {});
}
...
function install(filename, options) {
  ...
  var deps = read(filename);

  Object.keys(deps).forEach(function (name) {
    var pkg = new Package(name, deps[name], {
